...
$html    = $html . '<body><form action="/" method="POST"><input type="submit" value="MAXIMUM GO" /></form></body></html>';
...
while ( my $c = $d->accept ) {
	...
	if ( $r->method eq 'GET' and $r->uri->path eq "/" ) {
            ...
            if ( $conn_attempts == 1 ) { # Reset connection
                ...
                sleep(3);
                print "Resetting connection to test for RFC 2616 : 8.2.4 compliance...\n";
                $c->close;
                $c = $d->accept;
            } else {
                $c->send_response($resp);
